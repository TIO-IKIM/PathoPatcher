...
class TestPreProcessorTargetMagnificationDataset(unittest.TestCase):
    """Compare disk dataset with in memory dataset with selecting magnification"""

    @classmethod
    def setUpClass(cls) -> None:
        """Setup configuration"""
        check_test_database()
        cls.config = "./tests/static_test_files/preprocessing/target_magnification_dataset/config.yaml"
        ...
        cls.patch_config = LivePatchWSIConfig(
            ...
            target_mag=cls.opt_dict["target_mag"],
            patch_size=256,
            patch_overlap=0,
        )
