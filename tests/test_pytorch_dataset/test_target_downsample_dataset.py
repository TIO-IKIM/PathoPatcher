...
class TestPreProcessorTargetDownsampleDataset(unittest.TestCase):
    """Compare disk dataset with in memory dataset with selecting downsample factor"""

    @classmethod
    def setUpClass(cls) -> None:
        """Setup configuration"""
        check_test_database()
        cls.config = "./tests/static_test_files/preprocessing/downsample_dataset_dataset/config.yaml"
        ...
        cls.patch_config = LivePatchWSIConfig(
            ...
            downsample=cls.opt_dict["downsample"],
            patch_size=256,
            patch_overlap=0,
        )
