...
class PreProcessingYamlConfig(BaseModel):
    ...
    save_context_without_mask: Optional[bool]
...
class PreProcessingConfig(BaseModel):
    ...
    Args:
        ...
        save_context_without_mask (bool, optional): This is helpful for extracting patches, that are not within a mask, but needed for the Valuing Vicinity Segmentation Algorithms.
            This flag is specifically helpful if only fully annotated patches should be extracted from a region of interest (ROI) and their masks are stored,
            but also sourrounding neighbourhood patches (without mask) are needed. Defaults to False.
...
class PreProcessingParser(ABCParser):
    ...
    def __init__(self) -> None:
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=None,
            help="This is helpful for extracting patches, that are not within a mask, "
            "but needed for the Valuing Vicinity Segmentation Algorithms. "
            "This flag is specifically helpful if only fully annotated patches should be extracted "
            "from a region of interest (ROI) and their masks are stored, but also sourrounding "
            "neighbourhood patches (without mask) are needed.",
        )
        parser.add_argument(
            "--exclude_classes",
            action="append",
